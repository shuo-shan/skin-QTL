...
CPM <- CPM %>% dplyr::select(!unique(c(outlier_samples_metadata.krt$sample, 
                                       outlier_samples_metadata.mel$sample,
                                       genotype_outlier_samples_metadata$sample)))
log2CPM = log2CPM[, colnames(CPM)]
metadata = metadata %>% dplyr::filter(sample %in% colnames(CPM))
...
scenarios <- c(
  "MEL_PBS", "MEL_IFNG", "MEL_IFNB", "MEL_TNF",
  "MEL_PBS-IFNG", "MEL_PBS-IFNB", "MEL_PBS-TNF",
  "KRT_PBS", "KRT_IFNG", "KRT_IFNB", "KRT_TNF",
  "KRT_PBS-IFNG", "KRT_PBS-IFNB", "KRT_PBS-TNF",
  "FRB_PBS", "FRB_IFNG", "FRB_IFNB", "FRB_TNF",
  "FRB_PBS-IFNG", "FRB_PBS-IFNB", "FRB_PBS-TNF"
)
scenarios <- c(
...
length(expressed_genes_list[["MEL_PBS"]])  # number of expressed genes in MEL_PBS
...
for (sc in names(expressed_genes_list)) {
  ...
  if (grepl("-", sc)) {
    ...
    pattern <- paste0(ct, "_(", paste(conds, collapse="|"), ")")
    
    # Get matching samples
    samples <- grep(pattern, colnames(log2CPM), value = TRUE)
  } else {
    # Single-condition scenario
    samples <- colnames(log2CPM)[grepl(sc, colnames(log2CPM))]
  }
  ...
  expr_full <- log2CPM[genes, samples, drop = FALSE]
  ...
  cond <- factor(metadata[match(samples, metadata$sample), "condition"])
