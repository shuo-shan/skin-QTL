...
cat ${dir}/expressed_genes_tss.bed | grep -w ${chr} | awk -v p=${start_pos} '{if ($2>=p-500000 && $2<p+500000) print $5}' | sort | uniq > expressed_nearby_genes.txt
while read g;do
...
echo "check point 2: fetched nearby genes CPM values in PBS and IFN"; date;
...
while read g;do
	...
	Rscript /pi/manuel.garber-umw/human/skin/eQTLs/DREG/scripts/build_linear_model_log2FC_minimalModel08042023.R ${dir} ${snp} ${g} ${covariates} ${PBS_CPM} ${IFN_CPM} ${genotype}
