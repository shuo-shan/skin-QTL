...
###############################################################################################################################################################################
####### from this point, create a folder per TF2 of interest.
mkdir ${dir}/${TF2}
cd ${dir}/${TF2}

################# HIGH-Cor: TF2 in TF1-containing-Enhancers of genes that are highly correlated with TF1 ##################
# For TF1 highly correlating DE genes' enhancers that have high confidence that TF1 motif occurs, get the regions with high confidence that TF2 motif also occurs
regionF=${dir}/${TF1}_${TF1}HighCorrelationGenesEnhancers/fimo_topRanked_${TF1}_motif_center_flanking_1kb.bed
cat ${regionF} | awk '{OFS="\t"}{print $1"_"$2"_"$3"_"$6}' | sort | uniq | tr '_' '\t' | awk '{OFS="\t"}{print $1,$2,$3,$1"_"$2"_"$3,$4}' > regions.bed
run_FIMO_for_TF2 ${dir}/${TF2} ${TF2} regions.bed 100
regionTag=${TF1}TopMatching${TF1}HighCorrelationGenesEnhancers
Rscript ${scriptDir}/utility_calculate_bh_padj.R ${dir}/${TF2}/fimo_${TF2}.bed
cat padj_fimo_${TF2}.bed | awk '{if ($10 <= 0.05) print $0}' > fimo_topRanked_${TF2}_in_${regionTag}.bed
get_motif_center_and_flanking_region fimo_topRanked_${TF2}_in_${regionTag}.bed
rm fimo_${TF2}.bed padj_fimo_${TF2}.bed fimo_topRanked_${TF2}_in_${regionTag}.bed fimo_topRanked_${TF2}_in_${regionTag}_motif_center_flanking_1kb.bed regions.bed

################# Low-Cor: TF2 in TF1-containing-Enhancers of genes that are poorly correlated with TF1 ##################
# For TF1 poorly correlating DE genes' enhancers that have high confidence that TF1 motif occurs, get the regions with high confidence that TF2 motif also occurs
regionF=${dir}/${TF1}_${TF1}LowCorrelationGenesEnhancers/fimo_topRanked_${TF1}_motif_center_flanking_1kb.bed
cat ${regionF} | awk '{OFS="\t"}{print $1"_"$2"_"$3"_"$6}' | sort | uniq | tr '_' '\t' | awk '{OFS="\t"}{print $1,$2,$3,$1"_"$2"_"$3,$4}' > regions.bed
run_FIMO_for_TF2 ${dir}/${TF2} ${TF2} regions.bed 100
regionTag=${TF1}TopMatching${TF1}LowCorrelationGenesEnhancers
Rscript ${scriptDir}/utility_calculate_bh_padj.R ${dir}/${TF2}/fimo_${TF2}.bed
cat padj_fimo_${TF2}.bed | awk '{if ($10 <= 0.05) print $0}' > fimo_topRanked_${TF2}_in_${regionTag}.bed
get_motif_center_and_flanking_region fimo_topRanked_${TF2}_in_${regionTag}.bed
rm fimo_${TF2}.bed padj_fimo_${TF2}.bed fimo_topRanked_${TF2}_in_${regionTag}.bed fimo_topRanked_${TF2}_in_${regionTag}_motif_center_flanking_1kb.bed regions.bed
...
cd ${dir}
Rscript ${scriptDir}/utility_calculate_two_TF_motif_center_distance.R ${TF1} ${TF2}
