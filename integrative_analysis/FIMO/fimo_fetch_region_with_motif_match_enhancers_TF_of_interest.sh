...
# set-up
TF=MITF
regionF=${list_gene_enhancer}
regionTag=enhancerIn${celltype}
...
run_FIMO ${dir} ${TF} ${regionTag}.bed 1000
mv ${dir}/${TF}/fimo_${TF}.bed ./

# step 3. find enhancers/promoters that intersect with the fimo matching region
bedtools intersect -wa -wb -a ${regionTag}.bed -b fimo_${TF}.bed > ${regionTag}_with_${TF}_fimo_score.txt 
cat ${regionTag}_with_${TF}_fimo_score.txt | awk '{if ($13 < 0.00001) print $0}' | cut -f4 | sort | uniq > ${regionTag}_top.txt
grep -f ${regionTag}_top.txt ${regionF} | cut -f1 | sort | uniq -c > genes_regulated_by_${regionTag}_with_${TF}_fimo_p0.00001.txt
# output columns: motif_occurence, gene_name

# step 4. sometimes a region could have multiple motif matches. select the top performing motif match and output as table.
cat ${regionTag}_with_${TF}_fimo_score.txt | sort -g -k13 | awk '!seen[$4]++' > ${regionTag}_with_best_${TF}_fimo_score.txt
