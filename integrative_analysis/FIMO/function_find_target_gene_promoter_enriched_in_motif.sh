...
regions=$3 # full path of BED file
dict_linked_gene_cRE=/pi/manuel.garber-umw/human/skin/eQTLs/chromatin/RegulatoryElements/method4/promoters_and_enhancers_surrounding_genes_all3cts.bed
...
function run_FIMO {
        ...
        for f in fasta/split.*.fa;do
                ...
                fimo --oc ${thisDir}/${thisTF} --thresh 0.05 ${thisDir}/${thisTF}.meme ${thisDir}/${f}
...
#### MAIN
# generates a fimo_${TF}.bed with all regions containing motif occurence with qval <= 0.05
run_FIMO ${dir} ${TF} ${regions} 50
# overlap with promoter bed
bedtools intersect -wb -a ${dir}/${TF}/fimo_${TF}.bed -b ${dict_linked_gene_cRE} | awk '$17=="promoter"' | awk '{print $15}' | sort | uniq > ${dir}/FIMO_predicted_genes_with_${TF}_in_promoter.txt
# clean-up
cd ${dir}
rm -r ${dir}/${TF}
