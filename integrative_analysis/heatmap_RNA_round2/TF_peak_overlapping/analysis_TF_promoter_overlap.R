...
link_gene_promoter = readRDS(paste0(dir,"/../link_gene_promoter_within_300kbp_of_TSS_allcts.Rdata"))
load(paste0(dir,"/../myEnvironment_heatmap_RNAseq_DEgenes_14kmm-hc_04182023_padj0.05_log2FC1.5_avgCPM10.RData"))

#### compile promoter TF overlapping table ####
f = data.table::fread(paste0(dir,"/promoter_overlapping_TF_peaks_cleaned.txt"))
# aggregate data
aggregated_data <- f %>%
...
#### Heatmap of TF Promoter Overlaps: #1 cell-type-agnostic promoters ####
...
heatmap <- Heatmap(tf_matrix_ctagnostic, 
                   ...
                   border = TRUE,
                   row_title = "Genes", 
                   column_title = "Number of Overlap of TF and promoters of DE genes, cell-type-agnostic",
                   column_dend_height = unit(1, "cm"),
                   row_split = split_instructions.rna)  # Use row_split to divide rows based on split_instructions

pdf(paste0(dir,"/heatmap_promoter_TF_overlap_celltype_agnostic_binary.pdf"),width=24,height=30)
...
pdf(paste0(dir,"/heatmap_promoter_TF_overlap_celltype_agnostic_binary_rowclustered.pdf"),width=24,height=30)
draw(heatmap) # Adjust padding to ensure labels fit
dev.off()
#### Heatmap of TF Promoter Overlaps: #2 MEL promoters ####
# Specify split instructions
...
heatmap <- Heatmap(tf_matrix_MEL, 
                   ...
                   border = TRUE,
                   row_title = "Genes", 
                   column_title = "Number of Overlap of TF and promoters of DE genes, MEL promoters",
                   column_dend_height = unit(1, "cm"),
                   row_split = split_instructions.rna)  # Use row_split to divide rows based on split_instructions

# Draw the heatmap
pdf(paste0(dir,"/heatmap_promoter_TF_overlap_MEL.pdf"),width=24,height=30)
draw(heatmap) # Adjust padding to ensure labels fit
dev.off()
...
#### Heatmap of TF Promoter Overlaps: #2 FRB promoters ####
# Specify split instructions
...
heatmap <- Heatmap(tf_matrix_FRB, 
                   ...
                   border = TRUE,
                   row_title = "Genes", 
                   column_title = "Number of Overlap of TF and promoters of DE genes, FRB promoters",
                   column_dend_height = unit(1, "cm"),
                   row_split = split_instructions.rna)  # Use row_split to divide rows based on split_instructions

# Draw the heatmap
pdf(paste0(dir,"/heatmap_promoter_TF_overlap_FRB.pdf"),width=24,height=30)
draw(heatmap) # Adjust padding to ensure labels fit
dev.off()

#### Heatmap of TF Promoter Overlaps: #2 KRT promoters ####
# Specify split instructions
...
heatmap <- Heatmap(tf_matrix_KRT, 
                   ...
                   border = TRUE,
                   row_title = "Genes", 
                   column_title = "Number of Overlap of TF and promoters of DE genes, KRT promoters",
                   column_dend_height = unit(1, "cm"),
                   row_split = split_instructions.rna)  # Use row_split to divide rows based on split_instructions

# Draw the heatmap
pdf(paste0(dir,"/heatmap_promoter_TF_overlap_KRT.pdf"),width=24,height=30)
draw(heatmap) # Adjust padding to ensure labels fit
dev.off()
...
# Perform Fisher's Exact Test to compare the frequency of each TF's overlap with promoters in commonly-induced genes vs. cell-type-specific genes
#### PCA ####
# PCA of cell-type-agnostic promoters
