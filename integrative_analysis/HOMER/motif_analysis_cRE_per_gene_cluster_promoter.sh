...
cd ${dir}
# fetch corresponding gene promoters
...
grep -w -f ${dir}/temp.genes ${dict_gene_promoter} | cut -f2 > temp.this_gene_cluster_promoter
grep -w -f temp.this_gene_cluster_promoter ${cRE_bed} | awk '{OFS="\t"}{print $1,$2,$3,$4,".","+"}' > temp.plus.bed 
grep -w -f temp.this_gene_cluster_promoter ${cRE_bed} | awk '{OFS="\t"}{print $1,$2,$3,$4,".","-"}' > temp.minus.bed 
cat temp.plus.bed temp.minus.bed > this_gene_cluster_${region}.bed	
# fetch corresponding gene pruned-promoters
dict_gene_pruned_promoter=/pi/manuel.garber-umw/human/skin/eQTLs/chromatin/RegulatoryElements/method3/dictionary_induced_gene_pruned_promoter_links_allcts.txt
grep -w -f ${dir}/temp.genes ${dict_gene_pruned_promoter} | cut -f2 > temp.this_gene_cluster_pruned_promoter
grep -w -f temp.this_gene_cluster_pruned_promoter ${cRE_bed} | awk '{OFS="\t"}{print $1,$2,$3,$4,".","+"}' > temp.plus.bed
grep -w -f temp.this_gene_cluster_pruned_promoter ${cRE_bed} | awk '{OFS="\t"}{print $1,$2,$3,$4,".","-"}' > temp.minus.bed
cat temp.plus.bed temp.minus.bed > this_gene_cluster_${region}.bed
# clean-up
rm temp*
