...
# step6.0 Prepping vcf file to include only GWAS SNP and high LDs, and exclude ones with just one genotype
module load bcftools/1.16
mainvcfF=/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/vcf/merged/bd841628-fcc2-487a-8460-f5428237f0c9.merged.with_missing_allele.minAC3.AFtagged.vcf.gz
bcftools view --include ID==@gwas_snps_and_LD_tags_long.txt ${mainvcfF} -Oz -o ${dir}/output/gwas_snps_and_LD_tags.vcf.gz 
genotype_table=${dir}/output/gwas_snps_and_LD_tags.vcf.gz
...
split -l 1000 temp.filtered.genotype.bed temp.split.
sleep 10
rm commands.txt
conda activate fastQTL
for f in temp.split.*;do
  echo "date;Rscript ${dir}/scripts/further_filtering_SNPs.R ${dir} ${f} temp.header; date" >> commands.txt
done
while read c; do
  echo ${c} | bsub -W 8:00 -J filt -n 1 -R "span[hosts=1]" -R rusage[mem=2040] -q long -e "../log/further_filtering.%J%I.err" -o "../log/further_filtering.%J%I.out"
done < commands.txt
while [[ $(bjobs | grep filt | wc -l) != 0 ]] ; do echo $(bjobs | grep filt | wc -l) "jobs remaining"; sleep 5; done
# all outputs are named new.xxxx. without a header. join them and then add a header.
cat temp.header temp.split.* > filtered.genotype.bed
mv filtered.genotype.bed gwas-snps_filtered.genotype.bed
cat gwas-snps_filtered.genotype.bed | cut -f4 | sort | uniq > gwas_snps_with_LD_filtered.txt
bcftools view --include ID==@gwas_snps_with_LD_filtered.txt ${mainvcfF} -Oz -o ${dir}/output/gwas_snps_and_LD_tags_filtered.vcf.gz
tabix -p vcf /pi/manuel.garber-umw/human/skin/eQTLs/edQTL/output/gwas_snps_and_LD_tags_filtered.vcf.gz
...
# this results in a gwas_snps_and_LD_tags_filtered.vcf.gz that contain SNPs with all 3 genotypes, and at least 3 donors per genotype.
...
vcfF=${dir}/output/gwas_snps_and_LD_tags_filtered.vcf.gz
...
cd ${dir}/output
cat ${dir}/output/foreskin.edMat.10cov.20samps.noXYM.qqnorm.nominal | awk '$11!="NA"' > foreskin.edMat.10cov.20samps.noXYM.qqnorm.nominal.sig
