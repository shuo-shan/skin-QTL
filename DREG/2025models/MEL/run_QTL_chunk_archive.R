...
# Keep CPM columns that have metadata (for this celltype)
...
# Melt CPM to long (gene_id, sample, cpm) and merge metadata
message("[4/7] Reshaping CPM and merging metadata ...")
...
fml_str <- paste0("cpm ~ geno * condition ", cov_part, " + (1 | donor)")
...
fit_one <- function(snp, gene) {
  ...
  cols_needed <- c("sample","donor","condition","cpm", peer_cols, pc_cols)
  ...
  if (nrow(df) < 20) return(NULL)  # too small to be stable
  ...
  # Basic QC: enough genotype information?
  if (all(is.na(df$geno))) return(NULL)
  # Require at least 2 genotype groups and some minor allele support
  ...
  tr_summ <- as.data.frame(summary(tr))  # columns: condition, geno.trend, SE, df, t.ratio, p.value
