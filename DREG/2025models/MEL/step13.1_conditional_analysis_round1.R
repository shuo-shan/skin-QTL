...
if (length(args) < 4) {
  stop("Usage: Rscript step13.1_conditional_analysis_round1.R <CELLTYPE> <nPEER> <nGPC> <chunk_id>")
}
...
chunk_file <- paste0(dir,"/data/chunk/pair_chunk_",chunk_id,".txt")
GENO_FILE <- paste0(dir,"/data/chunk/genotype_pair_chunk_",chunk_id,".txt") # columns: CHR POS ID REF ALT sample1 sample2 ...
...
pairs <- fread(chunk_file, header=F) %>% 
...
gene_info <- fread("/pi/manuel.garber-umw/human/skin/eQTLs/DREG/2025models/MEL/data/gene_info.bed")
pairs <- pairs %>%
  left_join( . , gene_info, by=c("gene"="gene_name"))

snp_info <- fread(GENO_FILE) %>%
  dplyr::select(c("CHROM","POS","ID")) %>%
  set_colnames(c("SNP_chr","SNP_end","SNP"))
pairs <- pairs %>%
