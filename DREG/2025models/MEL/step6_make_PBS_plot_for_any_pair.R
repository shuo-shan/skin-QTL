...
if (length(args) < 3) {
  cat("
Usage:
  Rscript step6_make_PBS_plot_for_any_pair.R <gene> <snp>

Example:
  Rscript step6_make_PBS_plot_for_any_pair.R ITGA1 rs2548496 \\
", "\n")
...
STATS_FILE <- NULL
OUT_PDF <- paste0(basedir,"/temp_output/plot_PBS_",g,"_",snp,".pdf")
...
make_subpanel <- function(df, ct, stim, this_snp, this_gene){
  ...
  sub$condition <- factor(sub$condition, levels = unique(c("PBS", stim)))
...
make_plot <- function(this_gene, this_snp){
  ...
  df <- expr_long %>% filter(gene_id == this_gene) %>% filter(condition == "PBS")
