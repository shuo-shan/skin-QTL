...
# batch submit jobs to run gene reclassification on QTL genes (fdr 0.05)
...
# compile commands
CMD=${DIR}/step11_reclassify_commands.txt
rm -f ${CMD}
while read gene;do
	echo "bash ${DIR}/step11_reclassify_run_Rscript.sh ${ct} ${gene}" >> ${CMD}
done < ${gene_file}
...
Rscript ${DIR}/step11_reclassify_postRunCollection.R
