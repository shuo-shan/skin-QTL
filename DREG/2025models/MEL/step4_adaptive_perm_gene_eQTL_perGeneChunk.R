...
vst <- read_tsv(vst_file, show_col_types = FALSE) %>% dplyr::filter(gene %in% unique(pairs$gene))
...
run_one_gene <- function(this_gene,
                         pairs, genotype_all, vst, meta, peer, pcs,
  ...
  peer_use <- setdiff(colnames(peer), "sample")
  pc_use   <- setdiff(colnames(pcs),  "donor")
  ...
  donors <- unique(expr_donor$donor)
  ...
  donor_cols_present <- intersect(geno_sample_cols, donors)
  
  if (length(donor_cols_present) < 5) {
    return(tibble(
      celltype=ct, condition=condition, QTLtype=QTLtype, gene=this_gene,
      n_snps=length(snps), n_donors=length(donors),
      T_obs=NA_real_, perms=NA_integer_, exceed=NA_integer_,
  ...
  geno_sub <- genotype_all %>%
    ...
    dplyr::select(ID, all_of(donor_cols_present))
  
  # Build G: donors x SNPs
  # align SNP order + donor order
  G <- geno_sub %>%
    dplyr::arrange(match(ID, snps)) %>%
    dplyr::select(-ID) %>%
    as.matrix()
  ...
  gene_seed <- seed + (as.integer(abs(stats::runif(1) * 1e6)) %% 1000000)
