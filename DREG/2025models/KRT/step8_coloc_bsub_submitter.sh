...
#head -10 ${gene_file} > ${gene_file}.tmp && mv ${gene_file}.tmp ${gene_file}   # optional test
...
while read -r trait; do
  ...
  bsub -q short -W 0:30 -n 1 -R "span[hosts=1]" -R "rusage[mem=2000]" \
    -J "coloc_${trait}[1-${N}]%${MAX_CONCURRENT}" \
...
while [[ $(bjobs -w | grep coloc | wc -l) != 0 ]] ; do echo $(bjobs -w | grep coloc | wc -l) "jobs remaining"; sleep 5; done
