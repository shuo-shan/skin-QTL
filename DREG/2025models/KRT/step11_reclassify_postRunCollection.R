...
# ---- Are there nonDE eGenes that are super interesting? ----
ct="krt"
stim="ifng"
var <- ls(pattern = paste0("^deg.",ct,".",stim))
this_deg <- get(var)
colnames(this_deg)[1] <- "meta"
this_meta_dict <- convert_meta_genes(this_deg$meta)
this_deg <- this_meta_dict$gene

this_qtlgene <- f_unambiguous_genes_all_cyto %>%
  dplyr::filter(celltype==toupper(ct) & cytokine==toupper(stim)) %>%
  dplyr::filter(!gene %in% this_deg) 

length(unique(deg.nonde$gene))
# yes, SEPHS2:rs36093135 in KRT IFNG is interesting. it is not DE in 0/0 but induced in 0/1 and 1/1


# ---- compare distance to TSS in PBS-specific and cytokine-specific eGene lead SNPs ---- 
# The number of eGenes in each class (cytokine-specific, PBS-specific, shared if you include them)
# Be aware the gene sets may differ in composition — 
# and if a reviewer pushes back, matching on expression level quintile is a straightforward robustness check
#
# This lets you ask: 
# is the distal/enhancer enrichment driven by the DE genes (where you'd expect it), 
# or does it hold even for non-DE cytokine-specific eGenes (more surprising and interesting)?
...
ct="krt"
stim="ifng"
# 1. for this cytokine: PBS-specific eGene ----
this_f <- rbind(f.ifnb, f.ifng, f.tnf) %>%
  dplyr::filter(has_PBS_eQTL == "yes" & has_cytokine_eQTL == "no") %>%
  dplyr::filter(celltype == toupper(ct) & cytokine == toupper(stim))

# add chunk id per gene
this_gene_SNP <- distinct(this_f[, c("gene", "anchorSNP")])
this_gene_SNP <- this_gene_SNP %>%
  left_join(chunk_id_lookup, by = "gene") %>%
  mutate(chunk_id = sprintf("%03d", chunk))

# batch by chunk
dist_PBSeQTL <- this_gene_SNP %>%
  split(.$chunk_id) %>%
  lapply(function(chunk_df) {
    chunk_id <- unique(chunk_df$chunk_id)
    modelstats_file <- paste0(dir, "/results/result_", chunk_id, ".tsv")
    
    modelstats <- fread(modelstats_file) %>%
      dplyr::filter(gene %in% chunk_df$gene) %>%
      dplyr::filter(snp %in% chunk_df$anchorSNP) %>%
      dplyr::select(snp, gene, dist, contains("PBS")) %>%
      set_colnames(c("snp", "gene", "dist", "baseline", "beta", "p", "se")) %>%
      group_by(gene) %>%
      slice_min(p, n = 1, with_ties = FALSE) %>%
      ungroup()
    
    modelstats[, c("gene", "snp", "dist")]
  }) %>%
  bind_rows()

# 2. cytokine-specific eGene that's induced (Cytokine-specific eGenes that are DE) ---- 
var <- ls(pattern = paste0("^deg.",ct,".",stim))
this_deg <- get(var)
colnames(this_deg)[1] <- "meta"
this_meta_dict <- convert_meta_genes(this_deg$meta)
...
this_qtlgene_f <- f_unambiguous_genes_all_cyto %>%
  dplyr::filter(has_PBS_eQTL == "no" & has_cytokine_eQTL == "yes") %>%
  dplyr::filter(celltype==toupper(ct) & cytokine==toupper(stim)) %>%
  dplyr::filter(gene %in% deg.up$gene) 

# add chunk id per gene
this_gene_SNP <- distinct(this_qtlgene_f[, c("gene", "anchorSNP")])
this_gene_SNP <- this_gene_SNP %>%
  left_join(chunk_id_lookup, by = "gene") %>%
  mutate(chunk_id = sprintf("%03d", chunk))

# batch by chunk
dist_cytoeQTL_induced <- this_gene_SNP %>%
  split(.$chunk_id) %>%
  lapply(function(chunk_df) {
    chunk_id <- unique(chunk_df$chunk_id)
    modelstats_file <- paste0(dir, "/results/result_", chunk_id, ".tsv")
    ...
    modelstats <- fread(modelstats_file) %>%
      dplyr::filter(gene %in% chunk_df$gene) %>%
      dplyr::filter(snp %in% chunk_df$anchorSNP) %>%
      dplyr::select(snp, gene, dist, contains(stim) & starts_with("eQTL")) %>%
      set_colnames(c("snp", "gene", "dist", "baseline", "beta", "p", "se")) %>%
      group_by(gene) %>%
      slice_min(p, n = 1, with_ties = FALSE) %>%
      ungroup()
...
# 3. cytokine-specific eGenes that are not DE ----
var <- ls(pattern = paste0("^deg.",ct,".",stim))
this_deg <- get(var)
colnames(this_deg)[1] <- "meta"
this_meta_dict <- convert_meta_genes(this_deg$meta)
...
this_qtlgene_f <- f_unambiguous_genes_all_cyto %>%
  dplyr::filter(has_PBS_eQTL == "no" & has_cytokine_eQTL == "yes") %>%
  dplyr::filter(celltype==toupper(ct) & cytokine==toupper(stim)) %>%
  dplyr::filter(!gene %in% this_deg_metaconverted$gene) 
...
# batch by chunk
dist_cytoeQTL_notDE <- this_gene_SNP %>%
  split(.$chunk_id) %>%
  lapply(function(chunk_df) {
    chunk_id <- unique(chunk_df$chunk_id)
    modelstats_file <- paste0(dir, "/results/result_", chunk_id, ".tsv")
...
# 4. baseline-shared eGene that's induced  ---- 
var <- ls(pattern = paste0("^deg.",ct,".",stim))
this_deg <- get(var)
colnames(this_deg)[1] <- "meta"
this_meta_dict <- convert_meta_genes(this_deg$meta)
...
this_qtlgene_f <- f_unambiguous_genes_all_cyto %>%
  dplyr::filter(has_PBS_eQTL == "yes" & has_cytokine_eQTL == "yes") %>%
  dplyr::filter(celltype==toupper(ct) & cytokine==toupper(stim)) %>%
  dplyr::filter(gene %in% deg.up$gene) 
...
# batch by chunk
dist_sharedeQTL_induced <- this_gene_SNP %>%
  split(.$chunk_id) %>%
  lapply(function(chunk_df) {
    chunk_id <- unique(chunk_df$chunk_id)
    modelstats_file <- paste0(dir, "/results/result_", chunk_id, ".tsv")
...
# 5. baseline-shared eGenes that are not DE ----
var <- ls(pattern = paste0("^deg.",ct,".",stim))
this_deg <- get(var)
colnames(this_deg)[1] <- "meta"
this_meta_dict <- convert_meta_genes(this_deg$meta)
...
# batch by chunk
dist_sharedeQTL_notDE <- this_gene_SNP %>%
  split(.$chunk_id) %>%
  lapply(function(chunk_df) {
    chunk_id <- unique(chunk_df$chunk_id)
    modelstats_file <- paste0(dir, "/results/result_", chunk_id, ".tsv")
...
# 6. Plot violin boxplot by group ----
dist_PBSeQTL$class <- "PBS-specific"
dist_cytoeQTL_induced$class <- "cytokine-specific"
dist_cytoeQTL_notDE$class <- "cytokine-specific"
dist_sharedeQTL_induced$class <- "shared"
dist_sharedeQTL_notDE$class <- "shared"

dist_PBSeQTL$DE <- "NA"
dist_cytoeQTL_induced$DE <- "induced"
dist_cytoeQTL_notDE$DE <- "notDE"
dist_sharedeQTL_induced$DE <- "induced"
dist_sharedeQTL_notDE$DE <- "notDE"

df <- rbind(dist_PBSeQTL,
            dist_cytoeQTL_induced,
            dist_cytoeQTL_notDE,
            dist_sharedeQTL_induced,
            dist_sharedeQTL_notDE)

saveRDS(df, file=paste0(dir,"/reclassified/data.frame_for_TSSdistance_stratefied.rds"))

df_plot <- df %>%
  mutate(
    abs_dist = abs(dist),
    class = factor(class, levels = c("PBS-specific", "shared", "cytokine-specific")),
    DE = factor(DE, levels = c("NA", "notDE", "induced"))
  )
# --- Plot 1 labels ---
class_medians <- df_plot %>%
  group_by(class) %>%
  summarise(median_dist = median(abs_dist), .groups = "drop")

class_labels <- setNames(
  paste0(class_medians$class, "\n(", scales::comma(round(class_medians$median_dist)), " bp)"),
  class_medians$class
)

p1 <- ggplot(df_plot, aes(x = class, y = abs_dist, fill = class)) +
  geom_violin(alpha = 0.7, trim = TRUE) +
  geom_boxplot(width = 0.15, outlier.shape = NA, fill = "white", alpha = 0.8) +
  scale_y_log10(labels = scales::comma) +
  scale_x_discrete(labels = class_labels) +
  scale_fill_manual(values = c("PBS-specific"      = "#4393c3",
                               "shared"            = "#878787",
                               "cytokine-specific" = "#d6604d")) +
  labs(x = NULL, y = "Distance to TSS (bp, log10)",
       title = "eQTL distance to TSS by gene class") +
  theme_bw() +
  theme(legend.position = "none")

# --- Plot 2 labels ---
de_labels <- df_plot %>%
  filter(class != "PBS-specific", DE %in% c("notDE", "induced")) %>%
  group_by(class, DE) %>%
  summarise(median_dist = median(abs_dist), .groups = "drop") %>%
  mutate(label = paste0(DE, "\n(", scales::comma(round(median_dist)), " bp)"))

# build a per-facet labeller: list of named vectors, one per class
de_label_vectors <- de_labels %>%
  split(.$class) %>%
  lapply(function(x) setNames(x$label, x$DE))

# custom labeller function that picks the right vector per facet
de_labeller <- function(class_val) {
  function(x) de_label_vectors[[class_val]][x]
}

# since facet_wrap doesn't support per-facet x labels natively,
# we embed the median into the DE factor levels per class by re-leveling within each facet
df_plot2 <- df_plot %>%
  filter(class != "PBS-specific", DE %in% c("notDE", "induced")) %>%
  left_join(de_labels %>% select(class, DE, label), by = c("class", "DE")) %>%
  mutate(DE_label = factor(label, levels = unique(de_labels$label)))

p2 <- ggplot(df_plot2, aes(x = DE_label, y = abs_dist, fill = DE)) +
  geom_violin(alpha = 0.7, trim = TRUE) +
  geom_boxplot(width = 0.15, outlier.shape = NA, fill = "white", alpha = 0.8) +
  scale_y_log10(labels = scales::comma) +
  scale_fill_manual(values = c("notDE" = "#74add1", "induced" = "#f46d43")) +
  facet_wrap(~ class, scales = "free_x") +
  labs(x = NULL, y = "Distance to TSS (bp, log10)",
       title = "eQTL distance stratified by DE status",
       subtitle = "Within cytokine-specific and shared eGenes") +
  theme_bw() +
  theme(legend.position = "none")
...
# ---- compare overlap with ATACseq peaks in condition-specific eGene lead SNPs ---- 
# how do they overlap with constitutively open ATACseq peaks versus condition-specific (PBS- or cytokine- only) ATACseq peaks?
