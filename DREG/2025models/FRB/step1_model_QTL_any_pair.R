#!/usr/bin/env Rscript
# cis or transQTL: Mixed-effects eQTL / reQTL for any SNP-gene pair (sample-level PEERs)
...
if (length(args) < 7) {
  stop("Usage: step1_model_QTL_any_pair.R <DIR> <gene> <snp> <nPEER> <nGPC> <ct> <condition> <QTLtype>")
}

g <- args[1]
snp <- args[2]
nPEER <- as.integer(args[3]) # number of PEER factors to include
nGPC <- as.integer(args[4]) # number of genotype PCs to include
ct <- args[5] # MEL, KRT, FRB
this_condition <- args[6]
this_QTLtype <- args[7]
# 
# # for testing only
# g="RASIP1"
...
dir <- paste0("/pi/manuel.garber-umw/human/skin/eQTLs/DREG/2025models/",ct,"/results/temp/",g,"_",snp)
output_file <- paste0(dir,"/QTLmodeling_result_",g,"_",snp,"_",ct,"_",this_condition,"_",this_QTLtype,".txt")

## ------------------ Paths ------------------
# Expression + metadata
#CPM_FILE      <- "/pi/manuel.garber-umw/human/skin/eQTLs/RNA-Seq/analysis_07142025/CPM.sampleFiltered.metaConverted.txt"
VST_FILE <- paste0(dir,"/vst.txt")
