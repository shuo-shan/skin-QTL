...
run_one_gene_bonferroni <- function(this_gene,
  ...
  ms_g <- modelstats_subset %>%
    ...
    dplyr::select(snp, gene, beta, p, se, SNPtag,cis_trans_category, qc_n_per_condition_by_genotype) %>%
  ...
  tibble::tibble(
    ...
    lead_snp_beta = ms_g$beta[lead_idx],
    lead_snp_se = ms_g$se[lead_idx],
    ...
    qc_genotypes = ms_g$qc_n_per_condition_by_genotype[lead_idx]
  )
}
