...
#BSUB -q long
#BSUB -J QTL.FRB
#BSUB -o "/pi/manuel.garber-umw/human/skin/eQTLs/DREG/2025models/FRB/logs/step0_%J_%I.out"
#BSUB -e "/pi/manuel.garber-umw/human/skin/eQTLs/DREG/2025models/FRB/logs/step0_%J_%I.err"
...
#
## ------ chunk genes -------------- ####
## chunk gene list so that each chunk correspond to <= 100K gene_snp pairs
#echo "Chunking gene list with gene:snp cap of 100K per chunk..."
#
...
#        Rscript ${DIR}/step0_decide_gene_chunks.R ${ct}
#echo "Num gene chunks: $(ls -1 ${CHUNK_DIR}/gene_chunk_* | wc -l)"

# 3) for each gene chunk, extract all matching rows from the big pairs file (SNPs are bed 0-based coordinated now)
...
for gfile in ${CHUNK_DIR}/gene_chunk_*; do
  ...
  echo "Building ${out_pairs} from genes in ${gfile}"; date
...
# #BSUB -J QTL[1-273]%70 # create 273 jobs but run only 70 at a time concurrently, queue the rest
