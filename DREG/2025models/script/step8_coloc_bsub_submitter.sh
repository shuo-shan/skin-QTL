...
#head -5 ${gene_file} > ${gene_file}.tmp && mv ${gene_file}.tmp ${gene_file}   # optional test
...
while read -r trait; do
  ...
  bsub -q short -W 2:00 -n 1 -R "span[hosts=1]" -R "rusage[mem=2000]" \
    -J "MEL_coloc_${trait}[1-${N}]%${MAX_CONCURRENT}" \
...
while [[ $(bjobs -w | grep MEL_coloc | wc -l) != 0 ]] ; do echo $(bjobs -w | grep MEL_coloc | wc -l) "jobs remaining"; sleep 5; done
...
while read -r trait; do
        cd ${LOG_ROOT}/${trait}
        for f in *out; do
                grep "in cluster" ${f} | grep "Done" >> temp.donejobs.txt
        done

        echo "Total jobs: $(ls *.out | wc -l)  |    Finished jobs: $(wc -l temp.donejobs.txt)    |   Trait: ${trait}"
        rm temp.donejobs.txt
done < ${traits_file}
...
# Check out why some jobs failed
while read -r trait; do
        cd ${LOG_ROOT}/rerun/${trait}
        rm -f ${LOG_ROOT}/rerun/${trait}/temp.failedjobs.txt
        for f in *out; do
                if ! grep -q "Done" "$f"; then
                        echo "${f}" >> "${LOG_ROOT}/rerun/${trait}/temp.failedjobs.txt"
                fi
        done
        echo "Total jobs: $(ls *.out | wc -l)  |    Failed jobs: $(wc -l temp.failedjobs.txt)    |   Trait: ${trait}"
...
while read -r trait; do
  ...
  mkdir -p ${LOG_ROOT}/join
  mkdir -p ${LOG_ROOT}/join/${trait}
  rm -f ${CMD}
  ...
  bsub -q short -W 2:00 -n 1 -R "span[hosts=1]" -R "rusage[mem=5000]" \
    -J "${ct}_coloc_${trait}[1-${N}]%100" \
    -o "${LOG_ROOT}/join/${trait}/coloc_${trait}_%J_%I.out" \
    -e "${LOG_ROOT}/join/${trait}/coloc_${trait}_%J_%I.err" \
    ${DIR}/run_task.sh ${CMD}
done < ${traits_file}
