...
Example:
  Rscript adaptive_perm_gene_eQTL.R MEL_IFNB_eQTL.eigenMT.txt", "\n")
...
# read file 
f <- fread(paste0(dir,"/",infile)) %>% dplyr::select(-n_snps_total, -stage)

# performs BH multiple testing correction
f$q_gene = p.adjust(f$p_gene_eigenMT, method="BH")
f_gene_fdr05 <- f[which(f$q_gene<0.05),] %>% arrange(q_gene)
