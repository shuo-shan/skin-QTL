...
rm(chunk_id_lookup, chunk_id)

chunk_id_lookup <- data.table::fread(paste0(dir, "/data/gene_chunk_dict_old.txt"))
chunk_id <- unique(chunk_id_lookup[chunk_id_lookup$gene == this_gene, ]$chunk)
chunk_id <- sprintf("%03d", chunk_id)
...
pairs <- fread(pair_file, header = F) %>%
  set_colnames(c("SNP_ID","gene_name")) %>%
...
# ------------------------------
gene_info <- fread(paste0(dir,"/data/gene_info.bed")) %>% dplyr::filter(gene_name == this_gene)
chr <- unique(gene_info$gene_chr)
ciswindow_left <- unique(gene_info$gene_start) - 500000
ciswindow_right <- unique(gene_info$gene_start) + 500000
...
run_coloc_one_trait_preloadedGWAS <- function(
  ...
  # ----------------------------
  if (length(modelstats_file) > 1) {
    modelstats_all <- rbindlist(
      lapply(modelstats_file, fread, showProgress = FALSE),
      use.names = TRUE,
      fill = TRUE
    )
  } else {
