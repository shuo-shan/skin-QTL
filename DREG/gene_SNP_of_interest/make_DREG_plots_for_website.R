...
gene="HLA-DRB1"
snp="rs532098"
outDir=paste0("~/Downloads/nl/human/skin/eQTLs/DREG","/",snp)
...
###### load snp gene pair and make plots
df <- data.frame(snp=snp, gene=gene)
for (i in 1:nrow(df)) {
  rm(this_snp, this_gene, this_outDir, genotypeF, genotype)
  this_snp <- df[i,"snp"]
  this_gene <- df[i,"gene"]
  this_outDir=paste0("~/Downloads/nl/human/skin/eQTLs/DREG/gene_SNP_of_interest","/",this_gene,"_",this_snp)
  #system(paste0("mkdir ","~/Downloads/nl/human/skin/eQTLs/DREG/SNP_gene_of_interest"))
  #system(paste0("mkdir ",this_outDir))
  genotypeF=paste0(outDir,"/snp.bed")
