...
resultF="~/Downloads/nl/human/skin/eQTLs/DREG/KRT_minimal/KRT_PBS_eQTL_pval1E-5.txt"
n_gPCs=0
n_ePCs=0
...
colnames(f.krt)[c(3,4,5,6)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("minimalModel_reQTL_genotype_",x)))
colnames(f.krt)[c(7,8,9,10)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("featureSelected_reQTL_genotype_",x)))
colnames(f.krt)[c(11,12,13,14)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("minimalModel_PBSeQTL_genotype_",x)))
colnames(f.krt)[c(15,16,17,18)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("featureSelected_PBSeQTL_genotype_",x)))
colnames(f.krt)[c(19,20,21,22)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("minimalModel_IFNeQTL_genotype_",x)))
colnames(f.krt)[c(23,24,25,26)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("featureSelected_IFNeQTL_genotype_",x)))
...
f.krt.reQTL <- f.krt %>% dplyr::filter(reQTL_selectedFeatures_pval < 0.00001) %>% dplyr::arrange(reQTL_selectedFeatures_pval)
...
this.df <- f.krt %>% arrange(GENE,minimalModel_PBSeQTL_genotype_pval)
plot_list = list()
options(scipen = 0,digits=3) # turn on scientific notation with 3 digits
for (i in 1:nrow(this.df)) {
  this.snp=this.df[i,"SNP"] %>% as.character
...
pdf(paste0(outDir,"/plot/","KRT_PBSeQTL_pval1E-5.pdf"),width=15,height=8)
for (i in 1:length(plot_list)) {
  print(plot_list[[i]])
}
dev.off()
...
compile_SnpGeneEnhancerTable <- function(this.snp, this.gene){
  SnpGeneEnhancerTable <- dplyr::right_join(f.krt[,c("SNP","GENE")], 
...
this.gene <- "SYNE2"
this.gene.snps <- f.krt %>% dplyr::filter(GENE==this.gene) %>% pull(SNP)
...
all_genes <- unique(f.krt$GENE)
LDtraitTable <- data.frame()
for (i in 1:length(all_genes)) {
  this.gene <- all_genes[i]
  this.top.snp <- f.krt %>% dplyr::filter(GENE==this.gene) %>% top_n(., -1, minimalModel_PBSeQTL_genotype_pval) %>% pull(SNP)
