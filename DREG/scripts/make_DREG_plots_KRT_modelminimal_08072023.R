...
outDir="~/Downloads/nl/human/skin/eQTLs/DREG/KRT_minimal"
resultF="~/Downloads/nl/human/skin/eQTLs/DREG/KRT_minimal/KRT_reQTL_pval1E-5.txt"
n_gPCs=0
n_ePCs=0
genotype_table="~/Downloads/nl/human/skin/eQTLs/DREG/KRT_minimal/snps_near_expressed_genes.bed"
...
f.krt = data.table::fread(resultF, fill=TRUE) %>% unique() %>% set_rownames(paste0(.$V1, "_", .$V2))
#####
colnames(f.krt)[c(1,2)] <- c("SNP","GENE")
colnames(f.krt)[c(3,4,5,6)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("reQTL_genotype_",x)))
colnames(f.krt)[c(7,8,9,10)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("PBSeQTL_genotype_",x)))
colnames(f.krt)[c(11,12,13,14)] <- unlist(lapply(c("pval","pperm","beta","se"),function(x) paste0("IFNeQTL_genotype_",x)))

###### filter by effect size
f.krt.filter1 <- f.krt[which(abs(f.krt$reQTL_genotype_beta) > 0.35),]
...
enhancerDict <- read.table(enhancerF,header=F,sep="\t") %>% 
  ...
  dplyr::filter(gene %in% unique(f.krt$GENE))
...
SnpGeneEnhancerTable <- right_join(f.krt[,c("SNP","GENE")], 
...
for (i in 1:nrow(SnpGeneEnhancerTable)) {
  ...
  # this.snp <- "rs11056244"
  # this.gene <- "ERAP2"
  # this.region <- "merged_all_skin-eQTL_ATACseq_files_peak_83615"
...
this_SnpGeneEnhancerTable <- SnpGeneEnhancerTable %>% dplyr::filter()
# make a table of nearby enhancers of all ATACseq and H3K27ac ChIPseq signals
result.table = data.frame()
for (i in 1:nrow(SnpGeneEnhancerTable)) {
  this.snp <- as.character(SnpGeneEnhancerTable[i,"SNP"])
  this.gene <- as.character(SnpGeneEnhancerTable[i,"GENE"])
  ...
  cor.ATAC <- cor(as.numeric(scale(this.genotype)), z.diff.ATAC, method="pearson")
  cor.K27ac <- cor(as.numeric(scale(this.genotype)), z.diff.K27ac, method="pearson")
  
  this.result <- c(this.snp, this.gene, this.region, 
                   round(cor.ATAC,3), round(cor.K27ac,3),
                   paste(this.genotype, collapse="_"), 
  ...
  names(this.result) <- c("snp","gene","regionID",
                           "PCC_Genotype_ATAC-zDiff","PCC_Genotype_K27ac-zDiff",
                           "genotype_3donors",
                           "ATAC_PBS_3donors (CPM)", "ATAC_IFN_3donors (CPM)",
                           "K27ac_PBS_3donors (CPM)", "K27ac_IFN_3donors (CPM)") 
  result.table = rbind(result.table, this.result)
}
...
snpTag.filtered <- snpTag[which(snpTag$snp %in% unique(f.krt$SNP)),] %>% dplyr::arrange(desc(R))
  
  
###### plot interesting results
### 1. reQTL
snplst <- c("rs7142855")
this.df <- f.krt[which(f.krt$SNP %in% snplst),]

f.krt.reQTL <- f.krt.filter1 %>% dplyr::filter(reQTL_selectedFeatures_pval < 0.00001) %>% dplyr::arrange(reQTL_selectedFeatures_pval)
#write.table(f.krt.reQTL,paste0(outDir,"/reQTL_significant_results_pvalE-05_KRT.txt"),quote=F,row.names = F)

this.df <- f.krt %>% arrange(GENE,reQTL_genotype_pval)
plot_list = list()
...
for (i in 1:nrow(this.df)) {
  ...
  plot_reQTL_log2FC <- make_log2FC_plot(this.snp, this.gene, CPM.pbs.krt, CPM.ifn.krt)
...
pdf(paste0(outDir,"/plot/","KRT_reQTL_pval1E-5.pdf"),width=15,height=8)
...
# ### 2. PBS eQTL
# f.krt.PBSeQTL <- f.krt[which(f.krt$PBSeQTL_selectedFeatures_pval < 0.01),] %>% dplyr::arrange(PBSeQTL_selectedFeatures_pval)
# write.table(f.krt.PBSeQTL,paste0(outDir,"/PBSeQTL_significant_results_0.01_KRT.txt"),quote=F,row.names = F)
# this.df <- f.krt.PBSeQTL
# plot_list = list()
# options(scipen = 0,digits=3) # turn on scientific notation with 3 digits
# for (i in 1:nrow(this.df)) {
#   this.snp=this.df[i,"SNP"] %>% as.character
...
# 
# ### 3. IFN eQTL
# f.krt.IFNeQTL <- f.krt[which(f.krt$IFNeQTL_selectedFeatures_pval < 0.01),] %>% dplyr::arrange(IFNeQTL_selectedFeatures_pval)
# write.table(f.krt.IFNeQTL,paste0(outDir,"/IFNeQTL_significant_results_0.01_KRT.txt"),quote=F,row.names = F)
# this.df <- f.krt.IFNeQTL
# plot_list = list()
# options(scipen = 0,digits=3) # turn on scientific notation with 3 digits
# for (i in 1:nrow(this.df)) {
#   this.snp=this.df[i,"SNP"] %>% as.character
