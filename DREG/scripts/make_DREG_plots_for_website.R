...
make_all_the_plots <- function(snp, gene, outDir) {
  ...
  # 
  # paired plots
  png(paste0(outDir,"/pairedPlot_3donorColored_CPM.png"),width=300,height=200)
  print(pairedPlot_3donorColored_CPM); dev.off()
}

###### load snp gene pairs and make plots one pair at a time
snp_gene_pairs_F <- "~/Downloads/nl/human/skin/eQTLs/website/data/modeling_results/SNPGenePairs_QTLs_1E-5PvalCutoff_3cts.txt"
snp_gene_pairs <- data.table::fread(snp_gene_pairs_F, header=F) %>% 
  tidyr::separate(., col=V1, into=c("snp","gene"),sep="_") %>% 
  arrange(gene)

df <- snp_gene_pairs %>% dplyr::filter(grepl("^[A]", gene))
for (i in 1:nrow(df)) {
