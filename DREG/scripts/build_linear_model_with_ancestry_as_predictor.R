...
# Dir="~/Downloads/nl/human/skin/eQTLs/DREG/ancestry/KRT"
# snp="rs2910686"
# g="ERAP2"
...
build_linear_model_log2FC_nopermutation <- function() {
  ...
  flag <- TRUE
  tryCatch({
    # stability selection. only continue if ancestry (genotypePC1) is selected feature in 90% of the 100 5-fold-cross-validation runs. 
    ancestry_selected_table <- rep(FALSE,100)
    for (i in 1:100) {
      ...
      if ("1" %in% selected_features) {
        ancestry_selected_table[i] <- TRUE
      }
      ancestry_selection_freq <- length(which(ancestry_selected_table==T))/100
    }
  },
  ...
  if (!flag) {
    ...
    if ("1" %in% selected_features & ancestry_selection_freq >= 0.90){
      cat("ancestry is a stable selected feature \n")
      ...
      mat.fs <- mat.fs[,c("phenotype","1")]
      model.fs <- lm(phenotype ~ ., mat.fs)
      p.nominal <- summary(model.fs)$coefficients["`1`", "Pr(>|t|)"]
      p.adj <- NA
      beta <- summary(model.fs)$coefficients["`1`", "Estimate"]
      stdErr <- summary(model.fs)$coefficients["`1`", "Std. Error"]
      ...
      cat("ancestry genotype PC1 did not pass feature selection or isn't stable! \n")
      this_result_table=c(NA,NA,NA,NA,NA,NA,NA,NA)}
  }
...
build_linear_model_PBS_nopermutation <- function() {
  ...
  if (!flag) {
    ...
    # If ancestry is kept as a selected feature in at least 90% of the cv runs, fit the minimal model
    if ("1" %in% selected_features & ancestry_selection_freq >= 0.90){
      cat("ancestry is a stable selected feature \n")
      # Build the minimal linear regression model after feature selection
      mat.fs <- mat[,c(1,which(colnames(mat) %in% selected_features))] %>% as.data.frame()
      ...
      cat("ancestry (genotypePC1) did not pass feature selection or isn't stable! \n")
      this_result_table=c(NA,NA,NA,NA,NA,NA,NA,NA)
    }
  }
  return(this_result_table)
...
build_linear_model_IFN_nopermutation <- function() {
  ...
  flag <- TRUE
  tryCatch({
    # stability selection. only continue if ancestry is selected feature in 90% of the 100 5-fold-cross-validation runs. 
    ancestry_selected_table <- rep(FALSE,100)
    for (i in 1:100) {
      # Find the best lambda using cross-validation. lambda is a tuning parameter that controls the overall penalty strength.
      cv <- cv.glmnet(mat[,-1], mat[,1], alpha = 1, nfolds=5, standardize=TRUE)
  ...
  if (!flag) {
    ...
    # If ancestry is kept as a selected feature in at least 90% of the cv runs, fit the minimal model
    if ("1" %in% selected_features & ancestry_selection_freq >= 0.90){
      cat("ancestry genotypePC1 is a stable selected feature \n")
      # Build the minimal linear regression model after feature selection
      mat.fs <- mat[,c(1,which(colnames(mat) %in% selected_features))] %>% as.data.frame()
      mat.fs <- mat.fs[,c("phenotype","1")]
      model.fs <- lm(phenotype ~ ., mat.fs)
      ...
      cat("ancestry did not pass feature selection or isn't stable! \n")
      this_result_table=c(NA,NA,NA,NA,NA,NA,NA,NA)
      }
  }
  return(this_result_table)
