...
celltype=MEL
jobname=${celltype}
dir=/pi/manuel.garber-umw/human/skin/eQTLs/DREG/reQTL_betacomparison/MEL
cd ${dir}
mkdir plot log

# copy the covariate files, expressed gene files from basic DREG pipeline
olddir=/pi/manuel.garber-umw/human/skin/eQTLs/DREG/MEL_minimal
cp ${olddir}/expressed_genes_tss* ${dir}
cp ${olddir}/snps_near_expressed_genes.bed ${dir}

# get the eligible SNP:gene pair (425,315)
...
cat ${dir}/snp_gene_pairs.txt | awk -v dir=${dir} '{print "bash "dir"/pipeline_betacomparison_MEL_perSNPGenePair.sh "$0" "dir" masteroutput"}' > ${dir}/commands.txt
sleep 3
bash /pi/manuel.garber-umw/sshan/scripts/function_collapse_commands.sh ${dir} ${dir}/commands.txt ${dir}/commands.joined.txt 1000
sleep 5
while read c; do
        echo ${c} | bsub -W 96:00 -J ${jobname} -n 1 -R "span[hosts=1]" -R rusage[mem=800] -q long -e "${dir}/log/build_models_%J%I.err" -o "${dir}/log/build_models_%J%I.out"
