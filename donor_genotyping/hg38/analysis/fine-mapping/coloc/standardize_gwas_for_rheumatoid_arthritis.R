...
# ---------------- User metadata / paths ----------------
trait_name  <- "rheumatoid_arthritis"
type        <- "cc"                 # rheumatoid arthritis is case-control
prefer_snp  <- "rsid"               # or "variant_id"
infile      <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/rheumatoid_arthritis/GCST90479433.h.tsv.gz"
outfile     <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/rheumatoid_arthritis/standardized_GCST90479433.h.tsv.gz"

# Optional overrides (leave as NA_real_ if not used)
N_override      <- NA_real_
cases_total     <- 25533
controls_total  <- 290135
...
standardize_gwas_for_coloc <- function(gwas_path, trait_name,
  ...
  # normalize blanks -> NA (and we’ll write NA as literal "NA" in fwrite)
  ...
  # Here (GWAS-SSF) CI bounds for OR are typically on OR scale -> use log(hi/lo)
  ...
  # ---- s + s_source ----
  have_study_totals <- FALSE
  if (type == "cc") {
    # detection: per-SNP vs study totals repeated
    ...
    if (length(uc) == 1 && length(uC) == 1) {
      ...
      have_study_totals <- TRUE # we do have per-SNP counts
  ...
  # for cc: require s in (0,1) if study totals/per-snp provided; otherwise allow NA
  if (type == "cc") {
    if (isTRUE(have_study_totals) ||
        (is.finite(cases_total) && is.finite(controls_total) && (cases_total + controls_total) > 0)) {
