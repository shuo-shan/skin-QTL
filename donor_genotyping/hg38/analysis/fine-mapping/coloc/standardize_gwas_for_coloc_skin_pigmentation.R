...
# ----------------------------
trait_name  <- "skin_pigmentation"   # Skin colour (UKB data field 1717)
type        <- "quant"               # Pan-UKB quant trait
prefer_snp  <- "rsid"                # "rsid" or "variant_id"

infile  <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/skin_pigmentation/GCST90691754.h.tsv.gz"
outfile <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/skin_pigmentation/standardized_GCST90691754.h.tsv.gz"

# Pan-UKB quantitative traits are typically inverse-rank-normalized -> sdY ~ 1
sdY_override <- 0.556

# This file has no per-SNP N column; use study N from meta.yaml
N_override <- 415018
...
standardize_gwas_for_coloc <- function(gwas_path, trait_name,
  ...
  vid_col  <- pick("variant_id","variantID","hm_variant_id","hm_variantid","hm_code_variant_id","hm_codvariant_id")
  ...
  # cc-only placeholders (quant stays NA)
  ...
  # If N missing/bad: derive (cc) else N_override
  ...
  # ---- sdY (quant only; Pan-UKB INT => sdY=1)
  sdY_vec <- rep(NA_real_, nrow(dt))
  sdY_source <- rep("NA", nrow(dt))
  if (type == "quant" && is.finite(sdY_override)) {
  ...
  # Replace blank strings with NA (and later fwrite(na="NA"))
  ...
  if (type == "cc") {
    ...
    if (have_totals) out <- out[!is.na(s)]
  }
  ...
  # if quant type GWAS study, calculate sdY
...
cat(sprintf("[%s] sdY=%s (source=%s)\n", trait_name, as.character(out$sdY[1]), out$sdY_source[1]))
