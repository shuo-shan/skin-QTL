...
# ---------------------------
trait_name <- "basal_cell_carcinoma"
type <- "cc"  # "cc" or "quant"
prefer_snp <- "rsid"  # "rsid" or "variant_id"

infile  <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/basal_cell_carcinoma/GCST90475582.h.tsv.gz"
outfile <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/basal_cell_carcinoma/standardized_GCST90475582.h.tsv.gz"

# Optional overrides (use if needed)
N_override <- NA_real_
cases_total <- NA_real_     # e.g., 31142
controls_total <- NA_real_  # e.g., 404406
...
standardize_gwas_for_coloc <- function(gwas_path, trait_name,
  ...
  eaf_col <- pick("effect_allele_frequency","hm_effect_allele_frequency","eaf","EAF","af",
                  "case_af","control_af")  # we still prefer effect_allele_frequency if present
  ...
  # ---- start standardized table (keep only standardized columns later) ----
  out <- dt[, .(
    trait = trait_name,
    type  = type,
  ...
  # If N missing/bad and per-SNP num_cases/num_controls exist: derive
  ...
  if (type == "cc") {
    # detect whether per-SNP vs study totals repeated
...
out <- standardize_gwas_for_coloc(
  ...
  # (optional safety — these are also present in-file and constant here)
