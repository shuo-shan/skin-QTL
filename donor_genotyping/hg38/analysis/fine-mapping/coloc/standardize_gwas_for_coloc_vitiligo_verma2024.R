...
infile  <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/vitiligo_verma2024/GCST90476174.h.tsv.gz"
outfile <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/vitiligo_verma2024/standardized_GCST90476174.h.tsv.gz"
...
# (cc only; optional) Used only if file lacks usable num_cases/num_controls
...
standardize_gwas_for_coloc <- function(gwas_path, trait_name,
  ...
  # N and case/control
  n_col     <- pick("cum_eff_sample_size","effective_N","n_eff","n","N","sample_size")
  ...
  # IMPORTANT for this file: ci_lower/ci_upper are OR-scale CIs (positive),
  # and beta is logOR, so we infer SE on the log scale.
  ...
  # ---- s + s_source (cc only; per-SNP vs repeated totals detection) ----
  if (type == "cc") {
    out[, s := NA_real_]
    out[, s_source := "NA"]
  ...
  # ---- Replace blank strings with "NA" (requested) ----
