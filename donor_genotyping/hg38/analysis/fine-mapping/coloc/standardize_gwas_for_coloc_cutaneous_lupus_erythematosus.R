...
infile  <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/cutaneous_lupus_erythematosus/GCST90476182.h.tsv.gz"
outfile <- "/pi/manuel.garber-umw/human/skin/eQTLs/donor_genotyping/hg38/analysis/fine-mapping/coloc/cutaneous_lupus_erythematosus/standardized_GCST90476182.h.tsv.gz"
...
sdY_override <- NA_real_      # quant-only; leave NA for cc
...
standardize_gwas_for_coloc <- function(gwas_path, trait_name,
  ...
  # Normalize blanks -> NA (then later we write "NA" strings)
  ...
  # 1) explicit SE column (but only where finite >0)
  if (!is.na(se_col)) {
  ...
  # 3) p + beta: se = |beta| / z, where z from two-sided p
  ...
  # per-SNP N column if present and valid
  if (!is.na(n_col)) {
  ...
  # derive N from per-row cases/controls (only where N still missing/bad)
  ...
  if (type == "cc") {
    ...
    # detect whether num_cases/num_controls are per-SNP or study totals repeated
  ...
  if (type == "cc") {
    # require s in (0,1) if provided; allow NA only if truly no totals provided anywhere
    had_totals <- FALSE
  ...
  # (fwrite(na="NA") already covers true NA, but we also ensure no empty strings survive)
  for (ccol in c("trait","type","snp","rsid","variant_id","ea","oa","effect_scale","se_source","N_source","s_source","sdY_source")) {
  ...
  std_cols <- c(
    ...
    "eaf","MAF","N","N_source","num_cases","num_controls","s","s_source","sdY","sdY_source"
  )
  out <- out[, ..std_cols]
