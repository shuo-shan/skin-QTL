...
# =============================
# remove  unmapped, mate unmapped, too short (<10bp)
...
tmp_filt_orphanread_removed_bam_file="${filtered_alignment_dir}/${tmp_filt_bam_prefix}.orphanreadsremoved.bam"
...
samtools view -F 524 -f 2 -u ${raw_bam_file} | samtools sort -n -@ 10 -o ${tmp_filt_bam_file}  
...
samtools view -F 1804 -f 2 -u ${tmp_filt_fixmate_bam_file} | samtools sort -@ 10 -o ${tmp_filt_orphanread_removed_bam_file} -

# note: Remove aligned reads that are <10bp long
samtools view -h ${tmp_filt_orphanread_removed_bam_file} | \
awk 'BEGIN{OFS="\t"}
     /^@/ {print; next}
     {
         cigar = $6;
         ...
         if (len >= 10) print;
     }' |  samtools sort -@ 10 -o ${filt_bam_file} -
...
rm ${tmp_filt_orphanread_removed_bam_file}
