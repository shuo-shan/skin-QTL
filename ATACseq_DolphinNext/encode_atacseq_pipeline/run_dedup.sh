...
FINAL_BAM_INDEX_FILE="${deduped_bam_dir}/${FINAL_BAM_PREFIX}.bam.bai"
...
samtools sort -n --threads 2 ${FINAL_BAM_FILE} -O SAM  | SAMstats --sorted_sam_file -  --outf ${FINAL_BAM_FILE_MAPSTATS}
...
# col6: PBC1 = OnePair/DistinctReadPairs: PCR Bottleneck Coefficient 1, >0.9 is great (no bottle necking. 0.5-0.8 is moderate, and 0.8-0.9 is mild.
# col7: PBC2 = OnePair/TwoPair: PCR BottleneckCoefficient2, Ratio between unique and PCR duplicates, >>10 is great
samtools sort -n --threads 2 ${FILT_BAM_FILE} -o ${deduped_bam_dir}/${prefix}.srt.tmp.bam
