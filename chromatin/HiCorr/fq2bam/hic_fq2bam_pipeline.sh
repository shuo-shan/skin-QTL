...
dir=/nl/umw_manuel_garber/human/skin/eQTLs/literature/fibroblast/dixon_lung_fibroblast_imr90_hic
cd ${dir}
...
#outputname=Adrenal
hg38=/share/data/umw_biocore/genome_data/human/hg38/hg38
hg38Ind=/share/data/umw_biocore/genome_data/human/hg38_gencode_v34/hg38_gencode_v34.fa.fai
...
bed=${dir}/hindiii_frag.bed # <chr> <start> <end> <frag_id>
bed=${dir}/numerated_hindiii_frag.bed #cat hindiii_frag.bed | awk '{OFS="\t"}{print $1,$2,$3,NR,$5,$6}' > numerated_hindiii_frag.bed
outputname=IMR90

#### generate restriction enzyme digested fragmend bed file
#python3 /nl/umw_manuel_garber/human/skin/eQTLs/chromatin/HiCorr/fq2bam/hicpro_digest_genome.py /share/data/umw_biocore/genome_data/human/hg38/hg38.fa -r hindiii -o ${bed}
#cat hindiii_frag.bed | awk '{OFS="\t"}{print $1,$2,$3,NR,$5,$6}' > numerated_hindiii_frag.bed
#echo "done generating frag.bed";date;
## make file of fragments that overlap hg38 blacklisted regions
#wget https://github.com/Boyle-Lab/Blacklist/raw/master/lists/hg38-blacklist.v2.bed.gz
#gunzip hg38-blacklist.v2.bed.gz
#module load bedtools/2.29.2
#bedtools intersect -wa -a ${dir}/numerated_hindiii_frag.bed -b ${dir}/hg38-blacklist.v2.bed > ${HiCorrPath}/ref/HindIII/hg38.blacklist.frag 
...
#  let length=${length}-1
#  let trlen=${length}-36
#  bowtie -v 3 -m 1 --trim3 ${trlen} --best --strata --time -p 5 --sam ${hg38} ${fq1} ${expt}.R1.sam &
#  bowtie -v 3 -m 1 --trim3 ${trlen} --best --strata --time -p 5 --sam ${hg38} ${fq2} ${expt}.R2.sam &
#  wait
...
#  wait
#  $lib/pairing_two_SAM_reads.pl <(samtools view ${expt}.R1.sorted.bam) <(samtools view ${expt}.R2.sorted.bam) | samtools view -bS -t $hg38Ind -o - - > ${expt}.bam
