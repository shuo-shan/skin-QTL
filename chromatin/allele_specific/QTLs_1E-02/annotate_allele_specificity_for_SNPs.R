...
compile_allele_specificity <- function(outdir, SNPbed) {
	dir="/pi/manuel.garber-umw/human/skin/eQTLs/chromatin/allele_specific/QTLs_1E-02"
	qtl = SNPbed[, c("CHROM","POS","ID","REF","ALT")]
	qtl$start = qtl$POS - 1
	colnames(qtl) = c("chr","end","ID","REF","ALT","start")
	qtl = qtl[, c("chr","start","end","ID","REF","ALT")]
	...
	data.table::fwrite(result_final, file=paste0(outdir,"/QTL_1E-02_annotated_AlleleSpecificMark_summary.bed"),
                   quote=FALSE, sep="\t")
}
